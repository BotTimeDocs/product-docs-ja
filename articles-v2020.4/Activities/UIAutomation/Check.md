...
ラジオボタンとチェックボックスを選択またはクリア、デスクトップとブラウザをサポートします。
...
- **コントロール要素** ：変数を受信してチェックのターゲット要素とします。この項は**セレクタ**と二者択一で入力する必要があります。
- **[セレクタ](../Appendix/Selector.md?_v=v2020.4)** ：チェックのターゲット要素を示すために使用します。**要素を指定**をクリックすることで自動生成できます。文字列変数と文字列のみをサポートします。
...
- **チェック** ：単一選択ボックスまたはチェックボックスの選択状態はCheckチェック、Uncheckチェックがキャンセルされ、Toggele切り替えの三つがあります。

## 操作サンプル
1. **チェック**アクティビティをドラッグして要素を指定、ダウンメニューから"チェック"を選択。
...
3. ステップ1で指定した要素のページを開いたままにして、プロセスを実行して結果を確認する。
